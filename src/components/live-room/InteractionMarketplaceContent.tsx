...
export interface InteractionMarketplaceContentProps {
...
export function InteractionMarketplaceContent({
  initialTab = "gifts",
  creatorId,
  creatorName,
  walletBalance,
  ...
  return (
    <div className={`flex flex-col h-full ${className}`}>
      ...
      <div className="flex items-center gap-1 border-b border-zinc-800/80 bg-zinc-900/40 px-3 py-2 overflow-x-auto shrink-0 scrollbar-none">
        {[
      ...
      {feedbackMsg && (
        <div
          className={`flex items-center gap-2 px-4 py-2.5 text-xs font-bold shrink-0 animate-fade-in ${
      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4">
          ...
          <div className="space-y-4">
              ...
              {PRESET_GIFTS.map((g) => {
                ...
                return (
                  <button
                    ...
                    className={`relative flex flex-col items-center text-center p-3.5 rounded-2xl border transition-all group disabled:opacity-50 active:scale-95 ${
                      isLegendary
                        ? "bg-gradient-to-b from-amber-950/30 via-zinc-900 to-zinc-950 border-amber-500/50 hover:border-amber-400 shadow-lg shadow-amber-500/10"
            ...
            <div className="pt-1">
              ...
              <input
                ...
                placeholder="e.g., Amazing stream! Keep it up ✨"
          ...
          <div className="space-y-3">
            ...
            {interactions.length === 0 ? (
              <div className="py-12 text-center text-zinc-500 text-xs">
                No active interactions currently configured by broadcaster.
              </div>
            ) : (
                    ...
                    >
                      <div className="flex items-start justify-between gap-3 mb-2">
                        <div className="flex items-center gap-2.5 min-w-0">
                          <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-zinc-800 text-base border border-zinc-700/50">
                        ...
                        <div className="flex items-center gap-1 rounded-full bg-amber-500/20 px-2.5 py-1 text-xs font-black text-amber-400 border border-amber-500/30 shrink-0">
                          <Coins className="h-3 w-3" />
                          <span>{item.creditCost}</span>
                        </div>
                      </div>

                      <div className="flex items-center justify-between pt-2 mt-1 border-t border-zinc-800/80 text-[10px]">
                        <div className="flex items-center gap-2 text-zinc-400 flex-wrap">
                          ...
                          {(item as any).remainingQuantity !== null && (
                            <span className="text-amber-400 font-bold">
                              {(item as any).remainingQuantity} left
                            </span>
                          )}
                        </div>
                        ...
                        <button
                          ...
                          className="flex items-center gap-1.5 rounded-xl bg-gradient-to-r from-pink-600 to-rose-600 px-3.5 py-1.5 text-xs font-black text-white shadow-md shadow-pink-600/30 hover:from-pink-500 hover:to-rose-500 disabled:opacity-50 transition-all active:scale-95 shrink-0"
                        >
                          <Sparkles className="h-3 w-3" />
        ...
        {activeTab === "goal" && (
          <div className="space-y-4">
            <div className="rounded-3xl bg-zinc-900/80 border border-zinc-800 p-4 shadow-inner">
              <div className="flex items-center justify-between mb-2">
                <span className="text-xs font-bold text-white truncate mr-2">{goal.title}</span>
                <span className="rounded-full bg-amber-500/20 px-2.5 py-0.5 text-xs font-black text-amber-400 shrink-0">
                  {goal.percentage}%
                </span>
              </div>
              ...
              <div className="grid grid-cols-4 gap-2">
                {[25, 50, 100, 250].map((amt) => (
                  <button
                    ...
                    disabled={isProcessing}
                    className={`rounded-2xl py-2 text-xs font-black transition-all disabled:opacity-40 disabled:cursor-not-allowed ${
            ...
            <button
              ...
              disabled={isProcessing}
              className="w-full flex items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-pink-600 via-rose-600 to-amber-500 py-2.5 font-bold text-white shadow-xl shadow-pink-600/30 hover:opacity-95 active:scale-95 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
            >
              {isProcessing ? (
                <>
        ...
        {activeTab === "ppv" && (
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <span className="text-[11px] font-bold text-zinc-400 uppercase tracking-wider">
                Exclusive PPV Media Vault ({ppvVault.length})
              </span>
            ...
            {ppvVault.length === 0 ? (
              ...
              ppvVault.map((item) => (
                ...
                >
                  <div className="relative h-24 sm:h-20 w-full sm:w-28 shrink-0 rounded-xl overflow-hidden bg-zinc-950">
                    <img
                    ...
                    {!item.isUnlocked && (
                      <div className="absolute inset-0 flex items-center justify-center bg-black/40">
                        <span className="flex h-7 w-7 items-center justify-center rounded-full bg-pink-500/80 text-white shadow-lg">
                    ...
                    <div className="mt-2 flex items-center justify-between">
                      ...
                      {item.isUnlocked ? (
                        ...
                        <button
                          ...
                          disabled={isProcessing}
                          className="rounded-xl bg-gradient-to-r from-pink-600 to-rose-600 px-3 py-1.5 text-xs font-bold text-white shadow-md shadow-pink-600/20 hover:from-pink-500 hover:to-rose-500 disabled:opacity-50 transition-all"
                        >
                          Unlock Media
                        </button>
                      )}
        ...
        {activeTab === "vip" && (
          <div className="space-y-4">
            <div className="rounded-3xl border border-pink-500/40 bg-gradient-to-br from-pink-950/60 via-zinc-900 to-zinc-950 p-4 shadow-xl">
              <div className="flex items-center justify-between mb-2">
              ...
              <div className="space-y-1.5 mb-4 text-xs text-zinc-300">
              ...
              <div className="flex items-center justify-between pt-3 border-t border-zinc-800">
                ...
                <button
                  ...
                  disabled={isProcessing}
                  className="rounded-2xl bg-gradient-to-r from-pink-600 to-rose-600 px-4 py-2 text-xs font-bold text-white shadow-xl shadow-pink-600/30 hover:from-pink-500 hover:to-rose-500 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
                >
