import { NextRequest, NextResponse } from "next/server";
import { AdminAuthService } from "@/modules/admin/admin-auth.service";
import { AdminService } from "@/modules/admin/admin.service";

export async function GET(req: NextRequest) {
  try {
    await AdminAuthService.assertAdminAccess(req, "USERS_VIEW");

    const url = new URL(req.url);
    const query = url.searchParams.get("query") || undefined;
    const role = (url.searchParams.get("role") as any) || undefined;
    const kycStatus = (url.searchParams.get("kycStatus") as any) || undefined;
    const moderationState = (url.searchParams.get("moderationState") as any) || undefined;
    const isBannedParam = url.searchParams.get("isBanned");
    const isBanned = isBannedParam !== null ? isBannedParam === "true" : undefined;
    const limit = Number(url.searchParams.get("limit")) || 50;
    const offset = Number(url.searchParams.get("offset")) || 0;

    const result = await AdminService.searchUsers({
      query,
      role,
      kycStatus,
      moderationState,
      isBanned,
      limit,
      offset,
    });

    return NextResponse.json({ success: true, ...result });
  } catch (error: any) {
    return NextResponse.json(
      { error: error.message || "Failed to search users." },
      { status: error.statusCode || 500 }
    );
  }
}
